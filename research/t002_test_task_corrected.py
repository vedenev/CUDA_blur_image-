...
def run(image: np.ndarray, kernel_size: int, sigma: float):
    ...
    #print('xy_grid.type() =', xy_grid.type())
    ...
    return f(torch.from_numpy(np.expand_dims(np.swapaxes(image.astype(np.float32), 0, -1), axis=0))).cpu().numpy()
...
if __name__ == '__main__':
    ...
    res = run(img, 15, 3)
    img_output = np.swapaxes(np.squeeze(res.astype(np.uint8)), 0, -1)
    cv2.imshow('img', img_output)
    cv2.waitKey()
    cv2.imwrite('img_output.png', img_output)
    np.save('img_output.npy', img_output)
